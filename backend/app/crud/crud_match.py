...
def get_matches(db: Session):
        matches = db.query(Match).options(joinedload(Match.robot_performances)).all()
        return matches
...
def get_matches_by_event(event_id: int, db: Session):
        matches = db.query(Match).options(joinedload(Match.robot_performances)).filter(Match.event_id == event_id).all()
        return matches
